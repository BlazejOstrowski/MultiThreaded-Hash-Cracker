...
std::string Cracker::start(std::string hash){
	...
	for (auto&& thread : threadPool) // przejście przez wszystkie wątki w puli
		if (thread.joinable()) // jeśli wątek jest dostępny
			thread.join(); // dołączamy go
...
{
	// Szukanie zablokowanych wątków
	for (auto&& thread : threadPool)
	{
		if (thread.joinable()) // jeśli wątek nie jest zablokowany
	...
	// Szukanie niezablokowanych wątków
	for (auto&& thread : threadPool)
	{
		if (!thread.joinable()) // Jeśli wątek jest zablokowany
			continue; // przejdź do następnego wątku

		// Jeśli program nie przeszedł do następnego wątku
		thread.join(); // Blokuje znaleziony wątek
		thread = std::thread(function); // rozpoczyna wykonywanie funkcji na znalezionym wątku
		return;
	}
	return;
